 ...
 *
 */
public class TestErgebnisStoerfallImporter
extends CSVImporter{
	 ...
	 */
	public TestErgebnisStoerfallImporter(final ClientDavInterface dav, 
	...
	/**
	 * Bildet einen Ausgabe-Datensatz des Stoerfallindikator MARZ aus den Daten der aktuellen CSV-Zeile
	...
	/**
	 * Bildet einen Ausgabe-Datensatz des Stoerfallindikator NRW aus den Daten der aktuellen CSV-Zeile
	 * @return ein Datensatz der übergebenen Attributgruppe mit den Daten der nächsten Zeile
	 * oder <code>null</code>, wenn der Dateizeiger am Ende ist
	 */	
	...
	/**
	 * Bildet einen Ausgabe-Datensatz des Stoerfallindikator RDS aus den Daten der aktuellen CSV-Zeile
	 * @return ein Datensatz der übergebenen Attributgruppe mit den Daten der nächsten Zeile
	 * oder <code>null</code>, wenn der Dateizeiger am Ende ist
	 */	
	...
	/**
	 * Bildet einen Ausgabe-Datensatz der Stoerfallindikatoren aus den Daten der aktuellen CSV-Zeile
	 * @param offset Der Offset des zu nutzenden Stoerfallverfahrens  
	 * @return ein Datensatz der übergebenen Attributgruppe mit den Daten der nächsten Zeile
	 * oder <code>null</code>, wenn der Dateizeiger am Ende ist
	 */	
	private final Data getSF(final int offset){	
		...
		if(datensatz != null){
			...
			if(ZEILE != null){
				try{
					int situation = Integer.parseInt(ZEILE[17+offset]);

					
					DUAUtensilien.getAttributDatum("T", datensatz).asTimeValue().setMillis(INTERVALL); //$NON-NLS-1$
					DUAUtensilien.getAttributDatum("Situation", datensatz).asUnscaledValue().set(situation); //$NON-NLS-1$
					DUAUtensilien.getAttributDatum("Horizont", datensatz).asTimeValue().setMillis(0); //$NON-NLS-1$
					DUAUtensilien.getAttributDatum("Güte.Index", datensatz).asUnscaledValue().set(1); //$NON-NLS-1$
					DUAUtensilien.getAttributDatum("Güte.Verfahren", datensatz).asUnscaledValue().set(0); //$NON-NLS-1$
