...
implements IObjektWeckerListener{
	...
	private synchronized final ResultData getErgebnisAufBasisAktuellerDaten(){
		...
		if(aktuellesReferenzDatum == null || aktuellesReferenzDatum.getData() == null){
			analyseDatum.getTimeValue("T").setMillis(aktuellesReferenzDatum.getData().getTimeValue("T").getMillis()); //$NON-NLS-1$ //$NON-NLS-2$
			ergebnis = new ResultData(this.messQuerschnitt, MqAnalyseModul.PUB_BESCHREIBUNG, 
	 ...
	 * <code>VDelta</code>
	 * 
	 * @param attName der Name des Attributs, für das ein Ersatzdatum gefunden werden soll
	 ...
	 */
	private final ResultData getErsatzDatum(String attName){
		...
		if( !this.mqVorErfasst ){
			...
			if( !isDatumNutzbar(ersatzDatum, attName) ){
				ResultData mqDataNach = this.aktuelleMQAnalysen.get(this.mqv.getMQNach());
	 ...
	 */
	private final ResultData getAktuellesReferenzDatum(){
		ResultData ergebnis = null;
	...
	/**
	 * Setzt die Verkehrsstärke für diesen virtuellen Messquerschnitt in
	 * den Attributen <code>QKfz, QLkw</code> und <code>QPkw</code>
	 * 
	 * @param attName der Name des Attributs, für das die Verkehrsstärke gesetzt werden soll
	 */
	private final void setBilanzDatum(Data analyseDatum, String attName){
		QWert Q = null;
		
		/**
		 * 1. MQVor nicht direkt erfasst: Q(MQVor)=Q(MQMitte)+Q(MQAus).
		 * Wenn an MQMitte der jeweilige Wert nicht vorhanden ist, gilt: Q(MQVor)=Q(MQNach)+Q(MQAus)-Q(MQEin).
		 */
		if( !this.mqVorErfasst ){
			QWert QMitte = new QWert(this.aktuelleMQAnalysen.get(this.mqv.getMQMitte()), attName);
			QWert QAus = new QWert(this.aktuelleMQAnalysen.get(this.mqv.getMQAusfahrt()), attName);
			
			Q = QWert.summe(QMitte, QAus);
			
			if( Q == null || !Q.isExportierbarNach(analyseDatum) || !Q.isVerrechenbar() ){
				QWert QNach = new QWert(this.aktuelleMQAnalysen.get(this.mqv.getMQNach()), attName);
				QWert QEin = new QWert(this.aktuelleMQAnalysen.get(this.mqv.getMQEinfahrt()), attName);			
				
				if(QNach.isVerrechenbar() && QEin.isVerrechenbar()){
					if( Q == null || !Q.isExportierbarNach(analyseDatum) ){
						Q = QWert.differenz(
								QWert.summe(QNach, QAus),
								QEin
							);
					}else{
						/**
						...
						if(!Q.isVerrechenbar()){
							...
							if(dummy != null && dummy.isExportierbarNach(analyseDatum) && dummy.isVerrechenbar()){
								Q = dummy;
							}
						}			
					}
		...
		if( Q == null || !Q.isExportierbarNach(analyseDatum) || !Q.isVerrechenbar() ){
			...
			/**
			 * 2. MQMitte nicht direkt erfasst:
			 * Q(MQMitte)=Q(MQVor)-Q(MQAus). Wenn an MQVor der jeweilige Wert nicht vorhanden
			 * ist, gilt Q(MQMitte)=Q(MQNach)-Q(MQEin).
			 */
			if(!this.mqMitteErfasst){
				QWert QVor = new QWert(this.aktuelleMQAnalysen.get(this.mqv.getMQVor()), attName);
				QWert QAus = new QWert(this.aktuelleMQAnalysen.get(this.mqv.getMQAusfahrt()), attName);
			...
			if( Q == null || !Q.isExportierbarNach(analyseDatum) || !Q.isVerrechenbar() ){
				...
				/**
				 * 3. MQNach nicht direkt erfasst
				 * Q(MQNach)=Q(MQMitte)+Q(MQEin). Wenn an MQMitte der jeweilige Wert nicht
				 * vorhanden ist, gilt Q(MQNach)=Q(MQVor)+Q(MQEin)-Q(MQAus).
				 */
				if(!this.mqNachErfasst){
					QWert QMitte = new QWert(this.aktuelleMQAnalysen.get(this.mqv.getMQMitte()), attName);
					QWert QEin = new QWert(this.aktuelleMQAnalysen.get(this.mqv.getMQEinfahrt()), attName);
	...
	/**
	 * Erfragt, ob das übergebene Datum im Sinne der Wertersetzung brauchbar ist.
	 * Dies ist dann der Fall, wenn das Datum Nutzdaten enthält und dessen Datenzeit
	 * echt älter als die des letzten publizierten Analysedatums ist
	 * 
	 * @param datum ein Analysedatum eines MQ
	 * @return ob das übergebene Datum im Sinne der Wertersetzung brauchbar ist
	 */
	private final boolean isDatumOk(ResultData datum){
		boolean ergebnis = false;
		
		if(datum != null && datum.getData() != null){
			final long letzterAnalyseZeitStempel = this.letztesErgebnis == null?
												   -1:this.letztesErgebnis.getDataTime();
	...
	/**
	 * Erfragt, ob das übergebene Datum im übergebenen Attribut sinnvolle Nutzdaten
	 * (Werte >= 0 hat)
	 * 
	 * @param datum ein Analysedatum
	 * @param attName der Name des Attributs
	 * @return ob das übergebene Datum im übergebenen Attribut sinnvolle Daten
	 */
	private final boolean isDatumNutzbar(ResultData datum, String attName){
		boolean ergebnis = false;
