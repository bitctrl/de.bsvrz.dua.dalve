...
public class TestErgebnisPrognoseImporter extends CSVImporter {
	...
	/**
	 * Bildet einen Ausgabe-Datensatz der FS-Prognose/Analysewerte aus den Daten der aktuellen
	 * CSV-Zeile.
	 *
	 * @param attPraefix
	 *            Entweder <code>P</code> für einen Prognosedatensatz oder <code>G</code> für einen
	 *            Datensatz mit geglätteten Werten
	 * @param mode
	 *            the mode
	 * @return ein Datensatz der übergebenen Parametern mit den Daten der nächsten Zeile oder
	...
	private final Data getFSProgGlattDatensatz(final String attPraefix, final int mode) {
		...
		if (datensatz != null) {
			...
			if (ZEILE != null) {
				try {
					final int qB = Integer.parseInt(ZEILE[2 + offset + (mode * 2)]);
					final int vKfz = Integer.parseInt(ZEILE[8 + offset + (mode * 2)]);

					final int nErmFehl = DUAKonstanten.NICHT_ERMITTELBAR_BZW_FEHLERHAFT;
