...
public class TestFahrstreifenImporter extends CSVImporter {
	 ...
	 */
	public TestFahrstreifenImporter(final ClientDavInterface dav, final String csvQuelle)
			throws Exception {
		super(csvQuelle);
		if (DAV == null) {
			DAV = dav;
		...
		if (datensatz != null) {
			...
			if (ZEILE != null) {
				try {
					...
					final long tNetto = Long.parseLong(ZEILE[46 + (fsMulti * 2)]) * 1000;
					final String tNettoStatus = ZEILE[47 + (fsMulti * 2)];
					final int sKfz = 1;
